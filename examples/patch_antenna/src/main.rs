...
fn extract_pattern_3d<B: Backend>(
    device: &B::Device,
    fixture: &PatchFixture,
    f_res_ghz: f64,
    pml_thick: f64,
...
impl App for Args {
    fn run(self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if self.export_sweep {
            ...
            export_sweep::<B>(&device, &dir, self.f_start, self.f_stop, self.n);
            return Ok(());
        }
...
fn run_benchmark<B: Backend>(device: &B::Device, choice: FixtureChoice) {
    ...
    let rows = run_sweep::<B>(device, &fixture, freqs, pml_thick);
